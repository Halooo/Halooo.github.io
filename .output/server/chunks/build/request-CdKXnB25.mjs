import { c as useNamespace, i as isNumber, h as defaultNamespace, e as useZIndex, m as defaultInitialZIndex, o as namespaceContextKey, z as zIndexContextKey, p as isElement, q as fetchDefaults, u as useUserPinia, r as asyncDataDefaults, s as useRequestFetch, v as useModalPinia, w as useRuntimeConfig, x as useNuxtApp, y as createError } from './server.mjs';
import { inject, ref, computed, defineComponent, watch, renderSlot, openBlock, createElementBlock, normalizeClass, unref, createVNode, Transition, withCtx, withDirectives, createElementVNode, normalizeStyle, toDisplayString, vShow, shallowReactive, createBlock, createCommentVNode, resolveDynamicComponent, Fragment, withModifiers, isRef, getCurrentInstance, provide, isVNode, toValue, reactive, render, shallowRef, toRef, onServerPrefetch, toRaw } from 'vue';
import { G as hash } from '../runtime.mjs';
import { isClient, useEventListener, useResizeObserver, useTimeoutFn } from '@vueuse/core';
import { b as buildProps, e as definePropType, f as addUnit, w as withInstall, m as mutable, E as ElIcon, o as withInstallFunction, _ as _export_sfc } from './index-C8rz0VXy.mjs';
import { b as useSizeProp, u as useDeprecated, S as SIZE_INJECTION_KEY } from './index-URjNNh32.mjs';
import { i as iconPropType, T as TypeComponentsMap, a as TypeComponents } from './icon-CyxyPYk8.mjs';
...
const handleError = (response) => {
  ...
  const err = (text, name, code) => {
    const message2 = text || response.message || response.msg;
    if (name) {
      modalPinia.setModelOption({ message: message2, code });
      return modalPinia.setModalName(name);
    }
    ElMessage({ showClose: true, message: message2, type: "error", grouping: true });
  };
  const handleMap = {
    0: () => err("", "Fail"),
    401: () => err("", "Fail", 401)
  ...
  return {
    ...
    onRequest({ options }) {
      if (url.indexOf("http") !== 0)
        options.baseURL = option.baseURL || runtimeConfig.public.api.baseUrl;
      options.params = paramsSerializer(options.params);
      const isToken = option.isToken;
      options.headers = new Headers(options.headers);
      if (isToken)
        options.headers.set("token", `${userPinia.getToken}`);
    },
    // 响应拦截
    onResponse({ response }) {
      var _a, _b, _c;
      if (response.status === 200) {
        if (response._data) {
          if (response._data.code === 1)
            ...
            return Promise.reject((_a = response._data) != null ? _a : null);
          } else {
            handleError(response._data);
            return Promise.reject((_b = response._data) != null ? _b : null);
          }
        }
      }
      handleError(response._data);
      return Promise.reject((_c = response._data) != null ? _c : null);
    },
    // 错误处理
...
const useResFetch = (url, opts) => {
  return $fetch(url, fetchOption(url, opts));
};
const useCustomFetch = (url, opts) => {
  return useFetch(url, fetchOption(url, opts), "$mr5k66ooDp");
};

export { ElMessage as E, useLocale as a, useGlobalConfig as b, useCustomFetch as c, useEmptyValuesProps as d, useEmptyValues as e, entriesOf as f, getProp as g, useGlobalComponentSettings as h, keysOf as k, useResFetch as u };
